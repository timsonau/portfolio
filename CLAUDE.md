...
- `src/main.tsx` mounts `<Portfolio />`, the only top-level component. There is no routing; the whole site is one scrolling page (`src/Portfolio.tsx`) split into `<section>`s (`experience`, `articles`, `skills`, `education`) rendered by the components in `src/components/`.
- Layout is a single centered column (magazine/editorial style, see `docs/adr/0002-magazine-hero-single-column-layout.md`): a sticky horizontal top nav (`Navigation.tsx`), a full-viewport typographic `Hero`, then numbered chapter sections (`SectionHeading` takes an `index` prop like `"01"`) down one `max-w-3xl` column. `Navigation.tsx` renders anchor links (`#experience`, etc.) rather than using a router.
- `useActiveSection` (`src/hooks/`) drives the nav's active-state highlighting. It manually computes which section is "active" from scroll position (no IntersectionObserver for this part — it's a custom activation-line calculation) and locks itself during programmatic scrolls triggered by nav clicks so the highlight doesn't flicker mid-scroll. Its `SECTION_IDS` list must stay in sync with the sections actually rendered in `Portfolio.tsx`.
...
- `ArticlesSection` fetches the "Writing" list live at runtime via `useMediumArticles`, which pulls Tim's Medium RSS feed through `api.rss2json.com` (no build-time content). Loading/error/empty states are all handled in `ArticlesSection.tsx`.
- Theming is CSS-variable based, defined in `src/index.css`: a single light warm/editorial `:root` palette (see `docs/adr/0001-visual-redesign-warm-editorial-identity.md`) — there is no dark theme or theme switcher currently.
- Visual identity avoids bordered "card" boxes and tag-chip pills; rows are separated by hairline rule dividers, and tag/tech lists render as plain typographic runs (`Go · Kubernetes · Kafka`). The recurring accent device is `.pull-rule` (a left accent-colored rule used for pull-quote-style callouts) — reuse it rather than reintroducing chips/cards.
