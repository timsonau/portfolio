...
- `.editorial-card`, `.editorial-chip`, `.section-frame`, and `.photo-frame` (all box/chip treatments from ADR-0001) are removed from `index.css` as unused; `.drop-cap` and `.pull-rule` are added as the new recurring devices.
